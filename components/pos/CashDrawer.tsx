...
export default function CashDrawer({ onClose }: Props) {
  const { cashDrawer, openCashDrawer, closeCashDrawer, addCashMovement } =
    usePOSStore()
  ...
  const handleOpen = () => {
    ...
    // Get cashier from sessionStorage
    const user = JSON.parse(sessionStorage.getItem('pos_user') || '{}')
    openCashDrawer(amount, user?.name || 'Staff')
    setView('main')
    setOpeningCash('')
  }
            ...
            <div className='space-y-4'>
              <div className='rounded-lg p-3' style={{ background: '#F6F6F7' }}>
                ...
                >
                  ...
                  <span>
                    ₹{cashDrawer?.openingCash.toLocaleString('en-IN')}
                  </span>
                </div>
                <div
                  className='flex justify-between text-xs mb-1'
