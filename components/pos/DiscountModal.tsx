...
// Mock valid coupons — replace with real API later
const VALID_COUPONS: Record<
  string,
  { type: 'percent' | 'fixed'; value: number; label: string }
> = {
  SPORT15: { type: 'percent', value: 15, label: '15% off' },
  FLAT500: { type: 'fixed', value: 500, label: '₹500 off' },
  NEWUSER: { type: 'percent', value: 20, label: '20% off' },
    ...
    >
      ...
      >
        ...
        <div className='p-5 space-y-4'>
            ...
            >
              <div>
                ...
                <p className='text-xs' style={{ color: '#6D7175' }}>
                  -₹{(customDiscount + couponDiscount).toLocaleString('en-IN')}{' '}
                  off
                </p>
              </div>
              <button
          ...
          >
            {(['percent', 'fixed', 'coupon'] as DiscountType[]).map((t) => (
              <button
                key={t}
                onClick={() => setType(t)}
          ...
          {type === 'percent' && (
            <div>
              ...
              {percentVal && (
                <p className='text-xs mt-1' style={{ color: '#008060' }}>
                  = ₹
                  {Math.round(
                    (subtotal * (parseInt(percentVal) || 0)) / 100,
              ...
              <div className='flex flex-wrap gap-1.5 mt-2'>
                {Object.entries(VALID_COUPONS).map(([code, info]) => (
                  <button
                    ...
                    className='text-[11px] px-2 py-0.5 rounded-full border transition-colors hover:border-[#008060] hover:text-[#008060]'
