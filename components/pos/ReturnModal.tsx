...
export default function ReturnModal({ onClose }: Props) {
  const { completedOrders, processReturn } = usePOSStore()
  ...
  const handleProcessReturn = () => {
    ...
    processReturn(selectedOrder.id, items)
    setStep('success')
  }
                  ...
                  >
                    <div className='flex items-center justify-between mb-2'>
                      <div>
                        ...
                        <p className='text-xs' style={{ color: '#8C9196' }}>
                          ₹{item.product.price.toLocaleString('en-IN')} · Qty:{' '}
                          {item.quantity}
                        </p>
                      </div>
                      {/* Qty selector */}
        ...
        {step === 'success' && (
          <div className='p-5 text-center'>
            ...
            <p className='text-sm mb-5' style={{ color: '#6D7175' }}>
              Refund of ₹{refundTotal.toLocaleString('en-IN')} has been
