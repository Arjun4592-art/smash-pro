...
export default function SavedCarts({ onClose, onSave }: Props) {
  const { savedCarts, loadCart, deleteSavedCart, saveCart, items, total } =
    usePOSStore()
    ...
    >
      ...
      >
        ...
        {items.length > 0 && (
          <div
            className='px-5 py-3'
            style={{ borderBottom: '1px solid #E1E3E5', background: '#F6F6F7' }}
          >
            {!showSaveForm ? (
              <button
                onClick={() => setShowSaveForm(true)}
                className='w-full py-2 rounded-lg text-sm font-medium border transition-colors hover:bg-[#F2F7F5] hover:border-[#008060]'
                style={{ borderColor: '#E1E3E5', color: '#008060' }}
              >
                Save current cart (₹{total.toLocaleString('en-IN')})
              </button>
            ) : (
              <div className='flex gap-2'>
                <input
                  ...
                  placeholder='Cart name e.g. Rahul - hold'
                  className='flex-1 px-3 py-2 rounded-lg border text-sm outline-none focus:border-[#008060]'
              ...
              >
                <div className='flex-1 min-w-0'>
                  ...
                  <p className='text-xs' style={{ color: '#8C9196' }}>
                    ...
                    {new Date(cart.savedAt).toLocaleTimeString('en-IN', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
